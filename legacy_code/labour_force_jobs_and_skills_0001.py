...
# xlsx = pd.ExcelFile(links[1])
# xlsx.close()

with pd.ExcelFile(links[1]) as xlsx:
    for sheet in xlsx.sheet_names:
        print(sheet)
    df_unemp_rate = pd.read_excel(xlsx, sheet_name=0, header=3, index_col=1)
    df_unemp_no = pd.read_excel(xlsx, sheet_name=1, header=3, index_col=1)
    # df_labourf = pd.read_excel(xlsx, sheet_name=2, header=3, index_col=1)
    df_labourf = pd.read_excel(xlsx, sheet_name=2, header=3, index_col=[0,1], na_values='-')
    try:
        df_labourf = df_labourf.astype('Int64')
    except Exception as err:
        print(err)
        # Change to logging ref

# dfl = df_labourf.astype('Int64')
# dfl2 = df_labourf.astype('float32')
# dfl2 = df_labourf.astype('float32', errors='ignore')
